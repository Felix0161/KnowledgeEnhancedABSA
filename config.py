...
tf.app.flags.DEFINE_float('l2_reg', 0.001, 'l2 regularization') # was 0.00001
...
#knowledge graph
tf.app.flags.DEFINE_string("indicator_sentence", "_normal", "indicator for including only visible matrix (_without_softpos), only softpositions (_without_VM), both (_normal), or none (_without_VM_and_softpos)")
tf.app.flags.DEFINE_integer("hops", 0, "Knowledge depth level" )
tf.app.flags.DEFINE_integer("start", 0, "starting sentence number for creating the embeddings")
tf.app.flags.DEFINE_integer("end", 850, "starting sentence number for creating the embeddings")
tf.app.flags.DEFINE_string("embedding_path_weight_analysis", "data/programGeneratedData/" + "allEmbeddings" + str(FLAGS.year) + "_without_softpos"+ "_H=" + str(FLAGS.hops) + str(FLAGS.end) + ".txt", "embedding file path")
...
tf.app.flags.DEFINE_string("train_path", "data/programGeneratedData/" + "train_data"+str(FLAGS.year)+ "_H=" + str(FLAGS.hops) + "_targetkn_no_trainkn.txt", "train data path")
tf.app.flags.DEFINE_string("test_path", "data/programGeneratedData/" + "test_data"+str(FLAGS.year) + "_H=" + str(FLAGS.hops)+"_targetkn_no_trainkn.txt", "formatted test data path")
tf.app.flags.DEFINE_string("embedding_path", "data/programGeneratedData/" + "allEmbeddings" + str(FLAGS.year) + str(FLAGS.indicator_sentence)+ "_H=" + str(FLAGS.hops) + "_full.txt", "embedding file path")
